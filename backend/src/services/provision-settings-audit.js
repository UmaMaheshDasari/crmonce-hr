/**
 * Self-provisioning for the Settings Audit / history trail (hr_settingsaudits).
 * One APPEND-ONLY row per changed setting (scope / field / old / new / who / when /
 * effective date / rule version / reason). TEXT/MEMO columns, idempotent, lock-aware.
 * Mirrors provision-profile-audit.js.
 *
 *   Entity : hr_SettingsAudit (logical hr_settingsaudit, set hr_settingsaudits)
 *   Primary: hr_Name
 */
const axios = require('axios');
const d365 = require('./d365.service');

const ENTITY_LOGICAL = 'hr_settingsaudit';
const ENTITY_SCHEMA = 'hr_SettingsAudit';
const ENTITY_SET = 'hr_settingsaudits';
...
const COLUMNS = [
  str('hr_Scope', 'Scope', 30),            // payroll | attendance | company
  str('hr_Field', 'Field', 80),
  str('hr_FieldLabel', 'Field Label', 120),
  ...
  str('hr_RuleVersion', 'Rule Version', 20),
...
const ENTITY_BODY = {
  ...
  DisplayName: label('Settings Audit'), DisplayCollectionName: label('Settings Audits'),
  Description: label('Append-only history of Company / Payroll / Attendance setting changes.'),
  Attributes: [{
    '@odata.type': 'Microsoft.Dynamics.CRM.StringAttributeMetadata',
...
async function createSchema(log) {
  try { await post('EntityDefinitions', ENTITY_BODY); log?.info?.('[provision] created entity hr_SettingsAudit'); }
  catch (e) { const m = e.response?.data?.error?.message || e.message; if (isExists(m)) log?.info?.('[provision] entity hr_SettingsAudit already exists'); else if (isLocked(m)) throw Object.assign(new Error(m), { locked: true }); else throw new Error(m); }
  for (const c of COLUMNS) {
    try { await post(`EntityDefinitions(LogicalName='${ENTITY_LOGICAL}')/Attributes`, c); }
    catch (e) { const m = e.response?.data?.error?.message || e.message; if (isLocked(m)) throw Object.assign(new Error(m), { locked: true }); if (!isExists(m)) log?.warn?.(`[provision] settings-audit column ${c.SchemaName}: ${m}`); }
  }
}

async function ensureSettingsAuditTable(log = console, opts = {}) {
  ...
  for (;;) {
    try { await createSchema(log); log?.info?.('[provision] hr_settingsaudits ready'); return { status: 'created' }; }
    catch (e) {
      if (e.locked && retry && Date.now() - started + retryIntervalMs <= retryTimeoutMs) { log?.warn?.('[provision] settings-audit: Dataverse locked — retrying in 30s'); await sleep(retryIntervalMs); continue; }
      log?.warn?.(`[provision] could not auto-create hr_settingsaudits: ${e.message}`);
      return { status: e.locked ? 'locked' : 'unavailable', reason: e.message };
    }
  }
}

module.exports = { ensureSettingsAuditTable, ENTITY_SET };
