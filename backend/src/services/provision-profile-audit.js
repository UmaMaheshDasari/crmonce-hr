/**
 * Self-provisioning for the Employee Profile Audit trail (hr_profileaudits).
 * One row per changed field (who / when / old value / new value / action /
 * approved by). TEXT/MEMO columns, idempotent, lock-aware.
 *
 *   Entity : hr_ProfileAudit (logical hr_profileaudit, set hr_profileaudits)
 *   Primary: hr_Name
 */
const axios = require('axios');
const d365 = require('./d365.service');

const ENTITY_LOGICAL = 'hr_profileaudit';
const ENTITY_SCHEMA = 'hr_ProfileAudit';
const ENTITY_SET = 'hr_profileaudits';
...
const COLUMNS = [
  ...
  str('hr_Action', 'Action', 30),          // updated | approved | rejected | changes_requested
  str('hr_UpdatedBy', 'Updated By', 200),
  str('hr_UpdatedOn', 'Updated On', 30),
...
const ENTITY_BODY = {
  ...
  DisplayName: label('Profile Audit'), DisplayCollectionName: label('Profile Audits'),
  Description: label('Employee self-service profile change history.'),
  Attributes: [{
    '@odata.type': 'Microsoft.Dynamics.CRM.StringAttributeMetadata',
...
async function createSchema(log) {
  try { await post('EntityDefinitions', ENTITY_BODY); log?.info?.('[provision] created entity hr_ProfileAudit'); }
  catch (e) { const m = e.response?.data?.error?.message || e.message; if (isExists(m)) log?.info?.('[provision] entity hr_ProfileAudit already exists'); else if (isLocked(m)) throw Object.assign(new Error(m), { locked: true }); else throw new Error(m); }
  for (const c of COLUMNS) {
    try { await post(`EntityDefinitions(LogicalName='${ENTITY_LOGICAL}')/Attributes`, c); }
    catch (e) { const m = e.response?.data?.error?.message || e.message; if (isLocked(m)) throw Object.assign(new Error(m), { locked: true }); if (!isExists(m)) log?.warn?.(`[provision] profile-audit column ${c.SchemaName}: ${m}`); }
  }
}

async function ensureProfileAuditTable(log = console, opts = {}) {
  ...
  for (;;) {
    try { await createSchema(log); log?.info?.('[provision] hr_profileaudits ready'); return { status: 'created' }; }
    catch (e) {
      if (e.locked && retry && Date.now() - started + retryIntervalMs <= retryTimeoutMs) { log?.warn?.('[provision] profile-audit: Dataverse locked — retrying in 30s'); await sleep(retryIntervalMs); continue; }
      log?.warn?.(`[provision] could not auto-create hr_profileaudits: ${e.message}`);
      return { status: e.locked ? 'locked' : 'unavailable', reason: e.message };
    }
  }
}

module.exports = { ensureProfileAuditTable, ENTITY_SET };
