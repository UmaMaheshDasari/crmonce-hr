...
export const attendanceApi = {
  ...
  sync: (from, to) => api.post('/attendance/sync', { from, to }),       // legacy direct-pull (office-network only)
  etimeSyncStatus: () => api.get('/attendance/etime-sync/status'),      // Office Sync Agent status (modern path)
  ...
  absentees: (params) => api.get('/attendance/absentees', { params }),   // params.includePending optionally adds pending-leave rows
...
export const compOffApi = {
  ...
  approve: (id, dayType) => api.patch(`/attendance/comp-off/${id}/approve`, dayType ? { dayType } : {}),
...
export const advanceApi = {
  ...
  update: (id, data) => api.patch(`/payroll/advances/${id}`, data),
...
export const requestLifecycleApi = {
  ...
  edit: (type, id, edits) => api.patch(`/requests/${type}/${id}`, { edits }),
