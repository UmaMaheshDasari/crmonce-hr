...
import { dashboardApi, attendanceApi, employeeApi, documentApi, leaveApi, lateLoginApi } from '../../api/endpoints';
...
export default function EmployeeDashboard() {
  ...
  // Late Login (this month) — pending/approved counts for the dashboard.
  const llMonth = format(new Date(), 'yyyy-MM');
  const { data: llRes } = useQuery({ queryKey: ['late-login-summary', 'self', llMonth], queryFn: () => lateLoginApi.summary({ month: llMonth }).then(r => r.data), enabled: !!user?.id });
  const ll = llRes || null;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:items-start">
        ...
        <div className="bg-white rounded-2xl border border-gray-200/70 shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <ClockIcon className="w-5 h-5 text-indigo-500" />
              <h2 className="text-base font-bold text-gray-900">Late Login · {format(new Date(), 'MMMM yyyy')}</h2>
            </div>
            <Link to="/late-login" className="text-xs font-semibold text-indigo-600 hover:text-indigo-700">Manage →</Link>
          </div>
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-2.5">
            {[
              { label: 'Requests', value: ll.total || 0, tone: 'bg-gray-50 text-gray-800' },
              { label: 'Pending', value: ll.pending || 0, tone: 'bg-amber-50 text-amber-700' },
              { label: 'Approved', value: ll.approved || 0, tone: 'bg-emerald-50 text-emerald-700' },
              { label: 'Rejected', value: ll.rejected || 0, tone: 'bg-red-50 text-red-700' },
              { label: 'Cancelled', value: ll.cancelled || 0, tone: 'bg-slate-100 text-slate-600' },
            ].map(c => (
              <div key={c.label} className={`rounded-xl px-3 py-2.5 ${c.tone}`}>
                <p className="text-xl font-extrabold tabular-nums leading-none">{c.value}</p>
                <p className="text-[12px] font-semibold mt-1 leading-tight">{c.label}</p>
